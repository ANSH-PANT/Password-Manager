    ...
    for (let i = 0; i < passwordData.length; i++) {
        ...
        html += `
            <tr>
                ...
                <td id="${i + "2"}" style="background-color: ${color};" ${editable}>${row.password} <i onClick="copyContent('${row.password}')" class="fa-regular fa-clipboard copy"></i></td>
...
const updatePasswordData = (index) => {
    const websiteData = document.getElementById(index + "0").innerText.replace(" Copy", "").trim();
    const usernameData = document.getElementById(index + "1").innerText.replace(" Copy", "").trim();
    const passwordData = document.getElementById(index + "2").innerText.replace(" Copy", "").trim();
